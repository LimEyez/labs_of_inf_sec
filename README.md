...
## Шаг 1:
Нажимаем на кнопку создания ключей

**Дополнительно:**
1. На этом этапе ключи генерируются до тех пор, пока произведение p * q = n не будет меньше 32000 и больше 1250. Связано это стем, что на этапе расшифровки символы исходного сообщения могут быть расшифрованы неверно. После такого топорного ограничения работает вроде как все хорошо *(проблема возникала в основном с русскими символами, т.к. коды символов слишком большие и на каком-то этапе происходит такая проблема)*

## Шаг 2:
Вводим поле под надписью: *Введите шифруемое сообщение* исходный текст (слишком большие предложения не стоит воодить, лучше ограничиться небольшим количеством символов *(ограничивался порверкой 3-4 предложениями)*).
...
**Дополнительно:**
1. После выполнения "Дополнительно" из ШАГА 1 все должно расшифровываться верно, но если вдруг такая проблема снова появится, то можно попробовать зановы выполнить все шаги начиная с "ШАГ 1",  пропустив "ШАГ 2".

## Шаг 5:
Нажимаем кнопку "Подписать"
*В поле ниже добавляется сигнатура сообщения*

**Дополнительно:**
1. В console.log выводится хэш сообщения с отметкой того, что это хэш на этапе "Подписи".

## Шаг 6:
Нажимаем кнопку "Проверка подписи"
*Ниже выводится статус проверки подписи*

**Дополнительно:**
1. В console выводится хэш сообщения и хэш восстановленный по сигнатуре с отметкой того, что это хэши на этапе "Провера подписи".


## Шаг 7:
На этом этапе можно проверить то, что проверка подписи работает правильно, для этого:
1. Изменить *Расшифрованное сообщение*,
2. Нажать на "Подписать" (не обязательно)
3. Нажать "Проверка подписи"

Проверка подписи будет безуспешной (доп. информация все также присутствует в console)

------------------------------------------------------------------------------------------------------------------
# Лабораторная работа №6 (Хэш-функция)

Используется (первая хэш-функция): H_i = E_(H_(i-1)) (M_i) (+) M_i

## Шаг 1:
Вводим ключ H_0 (Максимальная длина берется в 8 байт)

## Шаг 2:
Вводим сообщение

## Шаг 3:
Нажимаем "Получить хэш"
